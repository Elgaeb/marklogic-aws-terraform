module "server_group_3" {
  ...
  group_number = 3
  enable = var.server_group_3_node_count > 0

  availability_zone = var.availability_zones[var.server_group_3_subnet_index % length(var.availability_zones)]
  subnet_id = var.private_subnet_ids[var.server_group_3_subnet_index % length(var.private_subnet_ids)]

  node_count = var.server_group_3_node_count
  volume_size = var.server_group_3_volume_size
