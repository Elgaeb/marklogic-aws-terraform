module "server_group_5" {
  source = "./modules/server-group"

  group_number = 5
  enable = var.server_group_5_node_count > 0

  availability_zone = var.availability_zones[var.server_group_5_subnet_index % length(var.availability_zones)]
  subnet_id = var.private_subnet_ids[var.server_group_5_subnet_index % length(var.private_subnet_ids)]

  node_count = var.server_group_5_node_count
  volume_size = var.server_group_5_volume_size
  volume_type = var.server_group_5_volume_type
  volume_iops = var.server_group_5_volume_iops
  volume_encrypted = var.server_group_5_volume_encrypted
  volume_kms_key_id = var.server_group_5_volume_kms_key_id
  volume_count = var.server_group_5_volume_count
  instance_type = var.server_group_5_instance_type
