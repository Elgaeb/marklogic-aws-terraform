module "server_group_4" {
  source = "./modules/server-group"

  group_number = 4
  enable = var.server_group_4_node_count > 0

  availability_zone = var.availability_zones[var.server_group_4_subnet_index % length(var.availability_zones)]
  subnet_id = var.private_subnet_ids[var.server_group_4_subnet_index % length(var.private_subnet_ids)]

  node_count = var.server_group_4_node_count
  volume_size = var.server_group_4_volume_size
  volume_type = var.server_group_4_volume_type
  volume_iops = var.server_group_4_volume_iops
  volume_encrypted = var.server_group_4_volume_encrypted
  volume_kms_key_id = var.server_group_4_volume_kms_key_id
  volume_count = var.server_group_4_volume_count
  instance_type = var.server_group_4_instance_type
